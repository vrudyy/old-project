<?php
    ...
    if(isset($_POST["status"]) && $_POST["status"] != 0){
        $tutors = $dao->listAllWhere("Tutor", " WHERE `tutorStatusId` = ".$_POST["status"]." AND `clientId` = '$client->clientId';");
    }else{
    ...
    if(isset($_POST["course"]) && $_POST["course"] != "all"){
        $size = sizeof($tutors);
        for($i = 0; $i<$size; $i++){
            if(strpos($tutors[$i]->tutorApprovedCourses, $_POST["course"]) === false){
                unset($tutors[$i]);
            }
        }
    }
    ...
    $tutorStatuses = $dao->listAll("TutorStatus");
        ...
        <div class="wrapper">
            ...
            <div class="content">
                <div class="home-title in-row">
                    <h3><?php echo $dic->translate("Tutors") ?></h3>
                    <?php
                    ...
                    ?>
                    <a href="newtutor.php"><?php echo $dic->translate("New Tutor") ?></a>
                </div>
                <?php
                    if(isset($_POST["filter"]) && $_POST["filter"]=='open'){
                        include_once 'content/tutors-filter.php';
                    }
                ?>
                <div class="in-row vsec cards" style="width:98%; margin: 20px 1%; padding: 10px;">
                    <?php
                    foreach($tutors as $t){
                        $c = $dao->get("Contact", $t->contactId, "contactId");
                        $status = $dao->get("TutorStatus", $t->tutorStatusId, "tutorStatusId");
                        echo '<div class="in-card-wrapper col3">';
                            echo '<div class="in-card in-row">';
                                echo '<div class="in-card-title col10">';
                                    echo '<form method="post" action="tutor.php?sec=approvedcourses">';
                                        echo '<input type="hidden" name="tutorId" value="'.$t->tutorId.'" />';
                                ...
                                echo '<div class="col10 in-card-details">';
                                    echo '<ul class="in-row">';
                                        ...
                                        echo '<li class="in-row"><span>'.$dic->translate("Status").':</span>'.$dic->translate($status->tutorStatusStatus).'</li>';
                                    echo '</ul>';
                                echo '</div>';
                                echo '<div class="col2 in-card-edit">';
                                    echo '<form action="edittutor.php" method="post" class="in-row in-card-edit">';
                                        echo '<input type="hidden" name="tutorId" value="'.$t->tutorId.'" />';
