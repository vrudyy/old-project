        ...
        echo '<form class="tutor-filter-status in-row" method="post">';
            ...
            if(!isset($_POST["status"]) || $_POST["status"] == 0){
                ...
                foreach($tutorStatuses as $t){
                    echo '<button name="status" value="'.$t->tutorStatusId.'">'.$dic->translate($t->tutorStatusStatus).'<i class="fas fa-circle"></i></button>';
                } 
            }else{
                echo '<button name="status" value="0">'.$dic->translate("All").'<i class="fas fa-circle"></i></button>';
                foreach($tutorStatuses as $t){
                    if($t->tutorStatusId == $_POST["status"]){
                        echo '<button class="tutor-filter-selected" name="status" value="'.$t->tutorStatusId.'">'.$dic->translate($t->tutorStatusStatus).'<i class="fas fa-check-circle"></i></button>';
                    }else{
                        echo '<button name="status" value="'.$t->tutorStatusId.'">'.$dic->translate($t->tutorStatusStatus).'<i class="fas fa-circle"></i></button>';
                    }
                }
